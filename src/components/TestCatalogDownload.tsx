...
TAPE-005,Scotch,Скотч прозрачный 19мм х 33м,шт,180,125,10%,,,,4606782987654,/images/tape.jpg`;
  ...
  const downloadCSV = () => {
    // Convert to Windows-1251 encoding for proper display in Excel
    ...
    link.setAttribute('download', 'test-catalog.csv');
            ...
            <Button 
              onClick={() => {
                const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify({
                  headers: ["Артикул", "Бренд", "Наименование", "Ед. (единицы измерения)", "Цена (Рекомендуемая)", "Цена дилер (по которой идет рассчет)", "Акция!!!", "% скидки", "Специальная цена!!!", "Упаковка (сколько единиц товара в большой коробке/средней коробки/малой коробки)", "Штрих-код", "Фото"],
                  data: [
                    ["RU-001", "Hatber", "Ручка шариковая синяя Megapolis", "шт", "120", "85", "", "", "", "", "4606782024689", "/images/pen-blue.jpg"],
                    ["NB-005", "Brauberg", "Блокнот А5 80 листов клетка", "шт", "280", "195", "15%", "", "", "", "4606788000567", "/images/notebook-a5.jpg"],
                    ["ST-012", "Office Space", "Степлер металлический №24/6", "шт", "450", "320", "", "", "290", "20 шт", "4606793245891", "/images/stapler.jpg"],
                ...
                downloadAnchorNode.setAttribute("download", "test-catalog.json");
              ...
              variant="outline"
            >
              <Icon name="FileJson" className="w-4 h-4 mr-2" />
              Скачать JSON (без проблем кодировки)
            </Button>
          </div>
        </div>
      </Card>
